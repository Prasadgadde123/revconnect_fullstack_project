...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    List<Post> findSoonToBePublishedPosts(@Param("author") User author, @Param("now") java.time.LocalDateTime now, @Param("cutoffTime") java.time.LocalDateTime cutoffTime);

    @Query("SELECT p FROM Post p WHERE p.deleted = false AND " +
            "p.author IN :users AND (p.scheduledAt IS NULL OR p.scheduledAt <= :now) " +
            "ORDER BY p.pinned DESC, p.createdAt DESC")
    Page<Post> findFeedPosts(@Param("users") List<User> users,
                             @Param("now") java.time.LocalDateTime now,
                             Pageable pageable);

    @Query("SELECT p FROM Post p WHERE p.deleted = false AND " +
            "p.author IN :users AND p.postType = :postType AND (p.scheduledAt IS NULL OR p.scheduledAt <= :now) " +
            "ORDER BY p.pinned DESC, p.createdAt DESC")
    Page<Post> findFeedPostsByType(@Param("users") List<User> users,
                                   @Param("postType") com.revconnect.enums.PostType postType,
                                   @Param("now") java.time.LocalDateTime now,
                                   Pageable pageable);

    @Query("SELECT p FROM Post p WHERE p.deleted = false AND " +
            "p.author IN :users AND p.author.role = :userRole AND (p.scheduledAt IS NULL OR p.scheduledAt <= :now) " +
            "ORDER BY p.pinned DESC, p.createdAt DESC")
    Page<Post> findFeedPostsByRole(@Param("users") List<User> users,
    ...
    List<Post> findRepostsByOriginalPostAndAuthor(@Param("originalPost") Post originalPost, @Param("author") User author);
