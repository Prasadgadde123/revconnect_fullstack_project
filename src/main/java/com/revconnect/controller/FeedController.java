...
public class FeedController {
    ...
    public String feed(@AuthenticationPrincipal User currentUser,
        ...
        if (currentUser.getRole() == com.revconnect.enums.UserRole.ADMIN)
            return "redirect:/admin";

        Page<Post> feedPage;
        if ((filterType == null || filterType.isBlank()) && (filterRole == null || filterRole.isBlank())) {
            feedPage = postService.getPersonalizedFeed(currentUser, page);
        } else {
            feedPage = postService.getFeedPosts(currentUser, page, filterType, filterRole);
        }
        ...
        if (currentUser.getRole() == com.revconnect.enums.UserRole.BUSINESS
    ...
    public String feedFragment(@AuthenticationPrincipal User currentUser,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(required = false) String filterType,
            @RequestParam(required = false) String filterRole,
            Model model) {
        if (currentUser == null) {
            return "null"; // Or handle gracefully in JS
        }
        Page<Post> feedPage;
        if ((filterType == null || filterType.isBlank()) && (filterRole == null || filterRole.isBlank())) {
            feedPage = postService.getPersonalizedFeed(currentUser, page);
